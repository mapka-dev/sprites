...
describe('ShelfPack', function() {
    describe('batch pack()', function() {
        test('batch pack() allocates same height bins on existing shelf', function(t) {
        ...
        test('batch pack() allocates larger bins on new shelf', function(t) {
        ...
        test('batch pack() allocates shorter bins on existing shelf, minimizing waste', function(t) {
            var sprite = new ShelfPack(64, 64),
                bins = [
                    { id: 'a', width: 10, height: 10 },
                    { id: 'b', width: 10, height: 15 },
        ...
        test('batch pack() accepts `w`, `h` for `width`, `height`', function(t) {
            var sprite = new ShelfPack(64, 64),
                bins = [
                    { id: 'a', w: 10, h: 10 },
                    { id: 'b', w: 10, h: 10 },
        ...
        test('batch pack() adds `x`, `y` properties to bins with `inPlace` option', function(t) {
            var sprite = new ShelfPack(64, 64),
                bins = [
                    { id: 'a', w: 10, h: 10 },
                    { id: 'b', w: 10, h: 10 },
        ...
        test('batch pack() skips bins if not enough room', function(t) {
        ...
        test('batch pack() results in minimal sprite width and height', function(t) {
            var bins = [
                { id: 'a', width: 10, height: 10 },
    ...
    describe('packOne()', function() {
        test('packOne() allocates bins with numeric id', function(t) {
            var sprite = new ShelfPack(64, 64);
            var bin = sprite.packOne(10, 10, 1000);
            expect(bin).toEqual({ id: 1000, x: 0,  y: 0, w: 10, h: 10, maxw: 10, maxh: 10, refcount: 1 }, 'packed bin 1000');
            expect(bin).toEqual(sprite.getBin(1000), 'got bin 1000');
        });

        test('packOne() allocates bins with string id', function(t) {
            var sprite = new ShelfPack(64, 64);
        ...
        test('packOne() generates incremental numeric ids, if id not provided', function(t) {
            var sprite = new ShelfPack(64, 64);
        ...
        test('packOne() does not generate an id that collides with an existing id', function(t) {
            var sprite = new ShelfPack(64, 64);
        ...
        test('packOne() does not reallocate a bin with existing id', function(t) {
            var sprite = new ShelfPack(64, 64);
        ...
        test('packOne() allocates same height bins on existing shelf', function(t) {
            var sprite = new ShelfPack(64, 64);
        ...
        test('packOne() allocates larger bins on new shelf', function() {
            var sprite = new ShelfPack(64, 64);

            //Shelf 1, 10x10 bin
            expect(sprite.packOne(10, 10)).toEqual({ id: 1, x: 0, y: 0,  w: 10, h: 10, maxw: 10, maxh: 10, refcount: 1 });
            //Shelf 2, 10x15 bin    
            expect(sprite.packOne(10, 15)).toEqual({ id: 2, x: 0, y: 10, w: 10, h: 15, maxw: 10, maxh: 15, refcount: 1 });
            //Shelf 3, 10x20 bin    
            expect(sprite.packOne(10, 20)).toEqual({ id: 3, x: 0, y: 25, w: 10, h: 20, maxw: 10, maxh: 20, refcount: 1 });
        });

        test('packOne() allocates shorter bins on existing shelf, minimizing waste', function(t) {
            var sprite = new ShelfPack(64, 64);
            ...
            expect(sprite.packOne(10, 20)).toEqual({ id: 3, x: 0,  y: 25, w: 10, h: 20, maxw: 10, maxh: 20, refcount: 1 });
            //Shelf 1, 10x9 bin            
            expect(sprite.packOne(10, 9)).toEqual({ id: 4, x: 10, y: 0,  w: 10, h: 9,  maxw: 10, maxh: 10, refcount: 1 });      
        });

        test('packOne() returns nothing if not enough room', function() {
            var sprite = new ShelfPack(10, 10);

            //First 10x10 bin
            expect(sprite.packOne(10, 10, 1)).toEqual({ id: 1, x: 0, y: 0, w: 10, h: 10, maxw: 10, maxh: 10, refcount: 1 });
            //Not enough room
        ...
        test('packOne() allocates in free bin if possible', function() {
            const sprite = new ShelfPack(64, 64);
            ...
            expect(sprite.packOne(10, 10, 4)).toEqual({ id: 4, x: 10, y: 0, w: 10, h: 10, maxw: 10, maxh: 10, refcount: 1 });
            //Freebins length 0
            expect(sprite.freebins.length).toEqual(0);
        });

        test('packOne() allocates new bin in least wasteful free bin', function() {
            var sprite = new ShelfPack(64, 64);
            sprite.packOne(10, 10, 1);
            ...
            expect(sprite.packOne(10, 13, 4)).toEqual({ id: 4, x: 0,  y: 10, w: 10, h: 13, maxw: 10, maxh: 15, refcount: 1 });
            //Freebins length 2
            expect(sprite.freebins.length).toEqual(2);
        });

        test('packOne() avoids free bin if all are more wasteful than packing on a shelf', function() {
            var sprite = new ShelfPack(64, 64);
            sprite.packOne(10, 10, 1);
            sprite.packOne(10, 15, 2);
            ...
            expect(sprite.packOne(10, 10, 3)).toEqual({ id: 3, x: 10,  y: 0, w: 10, h: 10, maxw: 10, maxh: 10, refcount: 1 });
            //Freebins still length 1
            expect(sprite.freebins.length).toEqual(1);
        });

        test('packOne() considers max bin dimensions when reusing a free bin', function() {
            var sprite = new ShelfPack(64, 64);
            sprite.packOne(10, 10, 1);
            sprite.packOne(10, 15, 2);
            sprite.unref(sprite.getBin(2));
            ...
            expect(sprite.packOne(10, 13, 3)).toEqual({ id: 3, x: 0, y: 10, w: 10, h: 13, maxw: 10, maxh: 15, refcount: 1 });
            // Freebins length 0
            expect(sprite.freebins.length).toEqual(0);
            sprite.unref(sprite.getBin(3));
            ...
            expect(sprite.packOne(10, 14, 4)).toEqual({ id: 4, x: 0, y: 10, w: 10, h: 14, maxw: 10, maxh: 15, refcount: 1 });
    ...
    describe('getBin()', function() {
        test('getBin() returns undefined if Bin not found', function(t) {
            var sprite = new ShelfPack(64, 64);
            expect(sprite.getBin(1)).toBeUndefined();
        });

        test('getBin() gets a Bin by numeric id', function() {
            var sprite = new ShelfPack(64, 64);
        ...
        test('getBin() gets a Bin by string id', function() {
            var sprite = new ShelfPack(64, 64);
            var bin = sprite.packOne(10, 10, 'foo');
    ...
    describe('ref()', function() {
        test('ref() increments the Bin refcount and updates stats', function(t) {
            var sprite = new ShelfPack(64, 64);
            var bin1 = sprite.packOne(10, 10, 1);
    ...
    describe('unref()', function(t) {
        test('unref() decrements the Bin refcount and updates stats', function(t) {
            var sprite = new ShelfPack(64, 64);
        ...
        test('unref() does nothing if refcount is already 0', function(t) {
            var sprite = new ShelfPack(64, 64);
            var bin = sprite.packOne(10, 10, 1);
    ...
    describe('clear()', function() {
        test('clear() succeeds', function(t) {
            var sprite = new ShelfPack(10, 10);
    ...
    describe('shrink()', function(t) {
        test('shrink() succeeds', function(t) {
            var sprite = new ShelfPack(20, 20);
            sprite.packOne(10, 5);
    ...
    describe('resize()', function() {
        test('resize larger succeeds', function() {
            var sprite = new ShelfPack(10, 10);

            expect(sprite.packOne(10, 10)).toEqual({ id: 1, x: 0, y: 0, w: 10, h: 10, maxw: 10, maxh: 10, refcount: 1 }, 'first 10x10 bin');
            expect(sprite.resize(20, 10)).toBeTruthy();
            expect(sprite.packOne(10, 10)).toEqual({ id: 2, x: 10, y: 0, w: 10, h: 10, maxw: 10, maxh: 10, refcount: 1 }, 'second 10x10 bin');
            expect(sprite.resize(20, 20)).toBeTruthy(); 
            expect(sprite.packOne(10, 10)).toEqual({ id: 3, x: 0, y: 10, w: 10, h: 10, maxw: 10, maxh: 10, refcount: 1 }, 'third 10x10 bin');
        });

        test('autoResize grows sprite dimensions by width then height', function() {
            var sprite = new ShelfPack(10, 10, { autoResize: true });
            ...
            expect(sprite.packOne(10, 10)).toEqual({ id: 3, x: 0,  y: 10, w: 10, h: 10, maxw: 10, maxh: 10, refcount: 1 }, 'third 10x10 bin');
            expect([sprite.w, sprite.h]).toEqual([20, 20]); 
            expect(sprite.packOne(10, 10)).toEqual({ id: 4, x: 10, y: 10, w: 10, h: 10, maxw: 10, maxh: 10, refcount: 1 }, 'fourth 10x10 bin');
            expect([sprite.w, sprite.h]).toEqual([20, 20]);
            expect(sprite.packOne(10, 10)).toEqual({ id: 5, x: 20, y: 0,  w: 10, h: 10, maxw: 10, maxh: 10, refcount: 1 }, 'fifth 10x10 bin');  
            expect([sprite.w, sprite.h]).toEqual([40, 20]);
        });

        test('autoResize accommodates big bin requests', function() {
            var sprite = new ShelfPack(10, 10, { autoResize: true });
            
            expect(sprite.packOne(20, 10)).toEqual({ id: 1, x: 0,  y: 0,  w: 20, h: 10, maxw: 20, maxh: 10, refcount: 1 }, '20x10 bin');
            expect([sprite.w, sprite.h]).toEqual([40, 10]);
            expect(sprite.packOne(10, 40)).toEqual({ id: 2, x: 0,  y: 10, w: 10, h: 40, maxw: 10, maxh: 40, refcount: 1 }, '40x10 bin');

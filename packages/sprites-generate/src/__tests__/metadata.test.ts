...
test("invalid metadata", () => {
  //rejects missing object
  expect(validateMetadata()).toMatchObject({ message: "image is invalid" });
  //rejects missing object
  expect(validateMetadata({})).toMatchObject({ message: "image has invalid metadata" });
  // rejects missing width
  expect(validateMetadata({}, {})).toMatchObject({ message: "image has invalid width" });
  // rejects zero width
  expect(validateMetadata({ width: 0 }, {})).toMatchObject({ message: "image has invalid width" });
  //rejects negative width
  expect(validateMetadata({ width: -3 }, {})).toMatchObject({ message: "image has invalid width" });
  // rejects missing height
  expect(validateMetadata({ width: 32 }, {})).toMatchObject({
    message: "image has invalid height",
  });
  //rejects height as object
  expect(validateMetadata({ width: 32, height: {} }, {})).toMatchObject({
    message: "image has invalid height",
  });
  // rejects negative height
  expect(validateMetadata({ width: 32, height: -32 }, {})).toMatchObject({
    message: "image has invalid height",
  });

  const img = { width: 24, height: 18 };

  //rejects invalid content area format
  expect(validateMetadata(img, { content: {} })).toMatchObject({
    message: "image content area must be an array of 4 numbers",
  });
  //rejects invalid content area format
  expect(validateMetadata(img, { content: [] })).toMatchObject({
    message: "image content area must be an array of 4 numbers",
  ...
  // rejects invalid content area size
  expect(validateMetadata(img, { content: [4, 4, 4, 4] })).toMatchObject({
    message: "image content area must be positive",
  });
  // rejects invalid content area size
  ...
  expect(validateMetadata(img, { content: [0, 0, 25, 18] })).toMatchObject({
    message: "image content area must be within image bounds",
  });
  // rejects invalid content area size
  ...
  // rejects invalid stretchX format
  expect(validateMetadata(img, { stretchX: {} })).toMatchObject({
    message: "image stretchX zones must be an array",
  });
  // rejects invalid stretchX format
  expect(validateMetadata(img, { stretchX: ["yes"] })).toMatchObject({
    message: "image stretchX zone must consist of two numbers",
  });
  // rejects invalid stretchX format
  expect(validateMetadata(img, { stretchX: [[]] })).toMatchObject({
    message: "image stretchX zone must consist of two numbers",
  ...
  //rejects invalid stretchX format
  expect(
    validateMetadata(img, {
      stretchX: [
        [4, 5],
        [6, null],
      ],
    }),
  ).toMatchObject({ message: "image stretchX zone must consist of two numbers" });
  // rejects invalid stretchX size
  expect(validateMetadata(img, { stretchX: [[4, 4]] })).toMatchObject({
    message: "image stretchX zone may not be zero-size",
  });
  // rejects invalid stretchX size
  ...
  expect(validateMetadata(img, { stretchX: [[-2, 2]] })).toMatchObject({
    message: "image stretchX zone must be within image bounds",
  });
  // rejects invalid stretchX size
  ...
  // rejects overlapping stretchX zones
  expect(
    validateMetadata(img, {
      stretchX: [
        [0, 2],
  ...
  ).toMatchObject({ message: "image stretchX zones may not overlap" });
  // rejects overlapping stretchX zones
  expect(
    validateMetadata(img, {
      stretchX: [
  ...
  // rejects unsorted stretchX zones
  expect(
    validateMetadata(img, {
      stretchX: [
        [18, 24],
